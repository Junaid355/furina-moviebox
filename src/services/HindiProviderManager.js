...
// Purged fake blockbuster mapping — Real Hollywood movies, anime, and series must NEVER be hijacked to play dummy rabbit videos or local wav files
export const BLOCKBUSTER_LOCAL_MEDIA_MAP = {};

export function resolveBlockbusterLocal() {
  return null;
}
...
export class LocalOwnedMediaProvider {
  ...
  canResolve(item) {
    ...
    // Studio playback mode is strictly reserved for custom videos explicitly created in the Movie Studio creator tool or sample
    if (item.id === 'studio_sample_1') return true;
    if ((item.isCustomMovie === true || item.isCustom === true) && item.isUserCreated === true) return true;
    if (String(item.id).startsWith('studio_') && item.isUserCreated === true) return true;
    return false;
  }

  resolve(item) {
    if ((item?.isCustom || item?.isCustomMovie) && (item?.isUserCreated || item?.id === 'studio_sample_1')) {
      return {
        providerId: this.id,
        ...
        hindi: {
          ...
          type: item.languages?.hi?.type || 'video/mp4',
        ...
        japanese: {
          ...
          type: item.languages?.ja?.type || 'video/mp4',
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        if (stored) {
          ...
          if (Array.isArray(list) && list.some((m) => m.isUserCreated && (m.id === item.id || Number(m.id) === numId) && m.languages?.hi?.url)) {
            return true;
          }
        }
      }
...
export function getPrioritizedAudioSources(item, langKey) {
  if (!item || (!item.isUserCreated && item.id !== 'studio_sample_1')) return [];
